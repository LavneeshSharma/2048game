let board;
    ...
    const API = {
      async getLeaderboard() {
        ...
        return JSON.parse(localStorage.getItem('global-leaderboard') || '[]');
      },
      async updateLeaderboard(newEntry) {
        await new Promise(resolve => setTimeout(resolve, 500));
        const currentLeaderboard = await this.getLeaderboard();
        currentLeaderboard.push(newEntry);
        currentLeaderboard.sort((a, b) => b.score - a.score);
        const updatedLeaderboard = currentLeaderboard.slice(0, 3); // Keep top 3 scores
        localStorage.setItem('global-leaderboard', JSON.stringify(updatedLeaderboard));
    ...
    function setupSwipeControls() {
      ...
      document.addEventListener("touchend", (e) => {
        ...
        enableUndo(); // Enable undo after each swipe
      });
    }
    ...
    function showNameInputDialog(gameState) {
      ...
      title.textContent = gameState === "win" ? "Congratulations!" : "Game Over";
      
      const message = document.createElement('p');
      message.textContent = gameState === "win" ? "You've reached 2048!" : "No more moves available.";
      ...
      nameInputDialog.appendChild(title);
      nameInputDialog.appendChild(message);
      nameInputDialog.appendChild(scoreDisplay);
    ...
    async function updateLeaderboard(playerName, playerScore) {
      ...
      leaderboard = await API.updateLeaderboard(newEntry);
      leaderboard = leaderboard.slice(0, 3); // Only keep top 3 scores
    ...
    function showMobileLeaderboard() {
      ...
      leaderboardOverlay.className = 'mobile-leaderboard-overlay';
      
      const leaderboardContent = document.createElement('div');
      leaderboardContent.className = 'mobile-leaderboard-content';
      
      const closeButton = document.createElement('button');
      closeButton.textContent = 'Close';
      closeButton.className = 'mobile-leaderboard-close';
      closeButton.addEventListener('click', () => {
    ...
    // Add a function to periodically update the leaderboard
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      const mobileLeaderboardButton = document.createElement('button');
      mobileLeaderboardButton.textContent = 'Leaderboard';
      mobileLeaderboardButton.className = 'mobile-leaderboard-button';
      mobileLeaderboardButton.addEventListener('click', showLeaderboardDialog);
      document.body.appendChild(mobileLeaderboardButton);
